...
> 情感内核不依赖 OpenClaw 运行时，可脱离网关直接测试。SDK 接线为实验性，集中在 `src/index.ts`。插件清单默认 `enabled: false`。

## 1. 实现边界

本工程把 **affect-core 情感系统框架规格 v0.2** 落地为 OpenClaw 原生插件。系统只调节代理的表达方式，绝不改变任务执行、工具调用、事实正确性和安全判断。所有插件钩子均采用失败开放（fail-open）策略：状态、模型评估、存储或表达注入发生异常时，代理照常运行，只是不注入情感舞台提示。

目标运行环境为 OpenClaw `2026.7.x`，全局扩展根目录默认 `~/.openclaw/extensions`，插件使用 `definePluginEntry` 与 `api.on(...)` 加载模型。在拿到对应版本完整、可编译的 SDK 声明包之前，事件桥接被隔离在 `src/index.ts`，核心逻辑维持为不依赖 OpenClaw 的纯 TypeScript。
...
| 文件 | 责任 | 关键审查点 |
|---|---|---|
| `types.ts` | 状态、人格、评估、关系、事件与适配器的唯一类型定义 | 不保存离散情绪名；PAD 与 mood 均有范围约束 |
...
| `clock.ts` | 时区、安静时段、睡眠窗口 | 非法 IANA 名回落 UTC，`tick()` 不会因坏配置抛错 |
...
入站消息事件名、前置提示注入钩子的返回字段、会话重置与心跳注册的精确签名，需以目标环境 `openclaw/plugin-sdk` 的完整声明最终校正。因此采用下面的策略。

1. `src/index.ts` 通过窄接口声明适配器形状，避免把不确定 SDK 字段扩散到情感内核。
2. 所有业务函数可脱离 OpenClaw 直接在 Node 测试中调用，审查者可验证数值正确性而不启动网关。
3. 入口中将已确认的工具前后置钩子接入；其他候选钩子以可选调用集中管理，缺失时仅跳过对应能力。
4. `openclaw.plugin.json` 提供完整配置 schema，但默认 `enabled: false`，防止装上后立即生效。
5. `DEPLOYMENT.md` 列出部署前必须执行的 SDK 类型核对、构建和插件检查步骤。
...
当前重点是验证数值模型、存储并发、降级路径和表达约束。启用前需要用实际安装版本的 `plugin-sdk` 复核入站消息、提示注入、会话复位与心跳的字段与返回值，并只修改 `src/index.ts` 适配层。这是显式记录的兼容性待办，不阻碍核心逻辑审查。
