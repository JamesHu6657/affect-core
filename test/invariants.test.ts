...
import { applyBondDelta, AFFECTION_CAP } from "../src/bonds.ts";
...
test("16. 任意状态下注入片段包含硬约束；强度 0 不含离散情绪名", () => {
  const needs = { contact: 1, recognition: 1, curiosity: 1, order: 1 };
  const neutral = renderStageNotes({ label: "平和", intensity: 0, warmth: "neutral", pace: "steady", assertiveness: "balanced", address: "formal", stage: "陌生", streak: 0, needs });
  assert.ok(neutral.includes(HARD_CONSTRAINTS));
  assert.ok(!/愉悦|受挫|低落|疲惫|谨慎|明朗/.test(neutral));
  const intense = renderStageNotes({ label: "受挫", intensity: 3, warmth: "cool", pace: "energetic", assertiveness: "tentative", address: "familiar", recentTag: "blame", stage: "亲近", streak: 3, needs });
...
test("L1 缓存命中不消耗小时配额", async () => {
  ...
  try {
    ...
    const first = { text: "这段关系对我很重要", userId: "u", receivedAt: start, kind: "message" as const };
    await core.onMessage(first);
    await core.onMessage(first);
    await core.onMessage({ text: "我们的关系建立在信任上", userId: "u", receivedAt: start, kind: "message" });
...
test("/mood 查询把状态文本返回给调用方", async () => {
  ...
  try {
    ...
    assert.ok(result?.reply?.includes("栞那现在"));
...
test("熟悉度不再随时间钝化；结转的负心境会降低用词温度", () => {
  const warm = { ...NEUTRAL_BOND, familiarity: 0.8, lastSeenAt: start };
  const later = applyBondDelta(warm, {}, start + 14 * 7 * 24 * 60 * 60_000, false);
  assert.equal(later.familiarity, warm.familiarity);
...
test("/mood 先 tick 再改时钟，空闲演化不会被抹掉", async () => {
  ...
  try {
    ...
    assert.ok(reply?.reply?.includes("栞那现在"));
